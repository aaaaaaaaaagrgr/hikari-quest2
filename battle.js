...
function makeFoe(id, idx, group){
  ...
  return {
    ...
    defMul:1, asleep:0, dead:false, shake:0, deadT:0, transformed:false,
  };
}
...
async function runBattle(group, opts={}){
  ...
  BV = {foes, boss:!!opts.boss, shake:0, flash:0, defX:1, ms:makeMemberState(), activeIdx:-1};
  ...
  for(;;){
    ...
    if(aliveFoes(foes).length===0){
      stopBgm();
      SFX.levelup();
      await msg(BV.boss ? `${foes[foes.length-1].name}を うちたおした！！` : 'まものたちを やっつけた！');
...
async function foeDown(f){
  if(f.next && !f.transformed){
    ...
    if(f.key==='noir'){
      await msg('「……ほう。 この わたしを ここまで おいつめるか。」',
        '「ならば みせてやろう…… めつぼうの すがたを！！」');
    } else {
    ...
    f.next = null; f.asleep = 0; f.defMul = 1;
...
async function battleSpellMenu(i, foes){
  const m = G.party[i];
  if(m.spells.length===0){ await msg(`${m.name}は まだ じゅもんを おぼえていない！`); return null; }
...
async function memberAct(i, action, foes, opts){
  ...
  if(action.t==='attack'){
    ...
    m.mp -= sp.mp;
    await msg(`${m.name}は ${sp.name}を となえた！`);
    SFX.spell();
    BV.flash = 8;
    if(sp.t==='dmg'){
      let target = action.target;
...
async function enemyAct(f){
  ...
  if(act.t==='atk'){
    ...
    await msg(`${f.name}は ${act.name}を はいた！`);
    ...
    await msg(`${f.name}は ${act.name}を まきちらした！`);
    let any = false;
...
const BATTLE_BG_COLOR = {
  ...
  icetemple:['#16243c','#3e5a7c'],
  altar: ['#1a1a30','#44446a'],
  darkcastle:['#170f20','#332441'],
};
